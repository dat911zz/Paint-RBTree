...
int searchMeanOfWord(BRTNode<Dictionary>* root, const char* x);
void insertXDictionary(BRTree<Dictionary>& tree);
void removeXDictionary(BRTree<Dictionary>& tree);
void editXDictionary(BRTree<Dictionary>& tree);
void deleteDictionary(BRTNode<Dictionary>*& root);
	...
	case 2:
		do
		{
			...
			case 3:
			{
				...
				cin >> mean;
				cout << "\nKet qua tim: ";
				if (searchMeanOfWord(dictionary.Root, mean) != 0)
				{
					cout << " Da tim thay ";
				}
				else
			...
			case 4:
				insertXDictionary(dictionary);
				break;
			case 5:
				removeXDictionary(dictionary);
				break;
			case 6:
				editXDictionary(dictionary);
				break;
			case 7:
				deleteDictionary(dictionary.Root);
				break;
			default:
				cout << "EXIT!" << endl;
				break;
...
int searchMeanOfWord(BRTNode<Dictionary>* root, const char* x)
{
	...
	int l = searchMeanOfWord(root->Right, x);
	int r = searchMeanOfWord(root->Left, x);
	return  stricmp(root->Info.mean, x) == 0 ? 1 + l + r : l + r;	
}
void searchMeanOfWord_2(BRTNode<Dictionary>* root, const char* x)
{
	if (root == NULL)
	{
		return;
	}
	if (stricmp(root->Info.mean, x) == 0)
	{
		cout << "\nDa tim thay";
		return;
	}
	searchMeanOfWord_2(root->Right, x);
	searchMeanOfWord_2(root->Left, x);
}
void inputDictionary(Dictionary& x)
{
	cout << "\nNhap word: ";
	cin >> x.word;
	cout << "\nNhap mean: ";
...
{
	Dictionary x;
	cout << "\nNhap word can chinh sua:";
	cin >> x.word;
	BRTNode<Dictionary>* p = search(tree.Root, x);
	if (p == NULL)
	{
		cout << "\nKhong tim thay!";
	}
